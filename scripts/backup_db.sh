#!/usr/bin/env bash
# backup_db.sh — Safe SQLite backup for LanceLLMot.
#
...
#   DB_PATH=/app/data/lancellmot.db BACKUP_DIR=/data/backups ./scripts/backup_db.sh
#
# Recommended cron (daily at 02:00):
#   0 2 * * * DB_PATH=/app/data/lancellmot.db BACKUP_DIR=/data/backups /path/to/scripts/backup_db.sh >> /var/log/lancellmot-backup.log 2>&1

set -euo pipefail

DB_PATH="${DB_PATH:-/app/data/lancellmot.db}"
